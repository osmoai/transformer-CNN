...
LOSSMAE = getConfig("Details", "lossMAE", "False");


LOSSMAE = True if LOSSMAE == "True" else False;
...
def buildNetwork():
    ...
    def mae_loss(prop):
       def loss(y_true, y_pred):
          y2 = y_true * l_ymask[prop] + y_pred * (1 - l_ymask[prop]);
          return tf.keras.losses.mae(y2, y_pred);
       return loss;
    ...
    for prop in props:
       if props[prop][2] == "regression":
          ...
          if LOSSMAE:
             print("lossMAE activated")
             losses.append(mae_loss(prop));
          else:
...
if __name__ == "__main__":
    ...
    # retrain the transformer part run this only once this is not for computing properties but to pretrain 
    if TRAIN == "True":
        ...
        if len(canon_pairs) > 0:
           ...
           # transformer decoder encoder parts
